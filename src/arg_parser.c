...
static const char* get_prefix(const arg_parser_t* root)
{
    return (root->arguments_prefix != NULL) ? root->arguments_prefix : DEFAULT_PREFIX;
}

static const char* get_short_prefix(const arg_parser_t* root)
{
...
static result_t parse_value(const char* str, arg_parser_value_type_t type, arg_parser_value_t* out)
{
...
static double value_to_double(arg_parser_value_t val, arg_parser_value_type_t type)
{
    switch (type)
    {
...
static result_t validate_restrictions(
    const arg_parser_arg_t* arg_def, arg_parser_value_t value)
{
    if (!arg_def->value_restrictions.enabled) return ok;

    const arg_parser_value_restrictions_t* r = &arg_def->value_restrictions;
...
static void print_command_help(const arg_parser_command_t* cmd, const char* path,
                               const char* long_pfx, const char* short_pfx)
{
    ...
    for (uint32_t i = 0; i < cmd->arguments_count; i++)
    {
        const arg_parser_arg_t* a = &cmd->arguments[i];
...
static void print_root_help(const arg_parser_t* root)
{
...
static void print_long_help_cmd(const arg_parser_command_t* cmd, const char* long_pfx,
...
static void print_long_help(const arg_parser_t* root)
{
    const char* long_pfx  = get_prefix(root);
...
static result_t parse_command_args(
    const arg_parser_command_t* cmd,
    const arg_parser_t* root,
    const char* path,
    int argc, char** argv,
    arg_parser_parsed_t* parsed)
{
    const char* long_pfx  = get_prefix(root);
    const char* short_pfx = get_short_prefix(root);
    ...
    arg_parser_parsed_arg_t storage[MAX_PARSED_ARGS];
    uint32_t count = 0;
    ...
    while (i < argc)
    {
        ...
        if (str_starts_with(token, long_pfx) && long_pfx_len > 0)
        {
            ...
            const arg_parser_arg_t* arg_def = NULL;
            uint32_t def_idx = 0;
        ...
        const arg_parser_arg_t* pos_def = NULL;
        uint32_t def_idx = 0;
    ...
    for (uint32_t j = 0; j < cmd->arguments_count; j++)
    {
        ...
        const arg_parser_arg_t* a = &cmd->arguments[j];
    ...
    {
        parsed->arguments = (arg_parser_parsed_arg_t*)malloc(
            count * sizeof(arg_parser_parsed_arg_t));
        if (parsed->arguments == NULL) return error;
        memcpy(parsed->arguments, storage, count * sizeof(arg_parser_parsed_arg_t));
    }
...
static const arg_parser_command_t* find_command(
    const arg_parser_command_t* commands, uint32_t count, const char* name)
{
    ...
    const void* context)
{
    ...
    {
        const arg_parser_command_t* sub = find_command(cmd->commands, cmd->commands_count, argv[0]);
        if (sub != NULL)
        {
            char sub_path[256];
...
result_t arg_parser_process_args(const arg_parser_t* root, int argc, char** argv)
{
    ...
    const arg_parser_command_t* cmd = find_command(root->commands, root->commands_count, argv[0]);
    if (cmd == NULL)
    {
